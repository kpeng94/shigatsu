...
public class Supply {
    ...
    public static void spreadSupplies(int threshold) throws GameActionException {
        ...
        for (int i = nearbyRobots.length; --i >= 0;) {
            ...
            if (robot.type == RobotType.MISSILE || robot.type.isBuilding)
                continue;
            double supplyLevel = robot.type == RobotType.LAUNCHER ? robot.supplyLevel / 3 : robot.supplyLevel; 
            totalSupply += supplyLevel;
            numBots++;
            if (supplyLevel < minSupply) {
