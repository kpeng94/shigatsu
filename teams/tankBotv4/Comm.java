...
public class Comm {
    ...
    public static final int COUNT_NEARRALLYPOINT_OFFSET = 17;
    ...
    public static int requestBlock(boolean isPermanent)
        ...
        Handler.rc.broadcast(free * BLOCK_SIZE, (isPermanent ? PERM_BLOCK_MASK
                : ACTIVE_BLOCK_MASK) + Clock.getRoundNum());
        return free;
    }
    ...
    public static void releaseBlock(int blockId) throws GameActionException {
        Handler.rc.broadcast(blockId * BLOCK_SIZE,
                Handler.rc.readBroadcast(FREEBLOCK_CHAN));
    ...
    public static void refreshBlock(int blockId) throws GameActionException {
        ...
        Handler.rc.broadcast(blockId * BLOCK_SIZE,
                (meta & 0xffff0000) + Clock.getRoundNum());
    }
    ...
    // Garbage collector
    public static void garbageCollect(int bytecodelimit)
            throws GameActionException {
        int lastGarbageCollected = Handler.rc
                .readBroadcast(LAST_GARBAGE_TURN_CHAN);
        ...
        while (Clock.getRoundNum() < bytecodelimit) {
            ...
            if (meta < PERM_BLOCK_MASK
                    && meta >= ACTIVE_BLOCK_MASK
                    && Clock.getRoundNum() - (meta & 0x0000ffff) > GARBAGE_THRESHOLD) {
                releaseBlock(cur);
            }
            if (cur == LAST_BLOCK) {
                cur = 0;
                Handler.rc.broadcast(LAST_GARBAGE_TURN_CHAN,
                        Clock.getRoundNum());
                break;
            }
        }
    ...
    // Writes a block
    public static void writeBlock(int blockId, int offset, int data)
            throws GameActionException {
        Handler.rc.broadcast(blockId * BLOCK_SIZE + offset, data);
    }

    // Writes a block prepended with current round
    public static void writeBlockWithRound(int blockId, int offset, int data)
            throws GameActionException {
        Handler.rc.broadcast(blockId * BLOCK_SIZE + offset,
                data + (Clock.getRoundNum() << 16));
    }

    // Reads whole block
    public static int readBlock(int blockId, int offset)
            throws GameActionException {
        return Handler.rc.readBroadcast(blockId * BLOCK_SIZE + offset);
    }

    // Reads the round the block was last written
    public static int readBlockRound(int blockId, int offset)
            throws GameActionException {
        return Handler.rc.readBroadcast(blockId * BLOCK_SIZE + offset) >>> 16;
    }

    // Reads only the data of the block
    public static int readBlockData(int blockId, int offset)
            throws GameActionException {
