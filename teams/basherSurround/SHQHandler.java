...
public class SHQHandler extends StructureHandler {
	...
	private static RobotType[] countTyps = {RobotType.BEAVER, RobotType.MINER, RobotType.BASHER, RobotType.TANK,
		RobotType.MINERFACTORY, RobotType.BARRACKS, RobotType.TANKFACTORY, RobotType.SUPPLYDEPOT};
	...
	protected static void init(RobotController rcon) throws GameActionException {
		...
		countChans = new int[]{Comm.getBeaverId(), Comm.getMinerId(), Comm.getBasherId(), Comm.getTankId(),
...
//		Supply.spreadSupplies(Supply.DEFAULT_THRESHOLD);
	...
	protected static void updateBuildStates() throws GameActionException {
		if (counts[2] >= 20 || Clock.getRoundNum() >= 500) {
			...
			Comm.writeBlock(Comm.getBarrackId(), 2, 5);
			Comm.writeBlock(Comm.getBasherId(), 2, 999);
			Comm.writeBlock(Comm.getSupplyId(), 2, 30);
		} else if (counts[4] == 1) {
			Comm.writeBlock(Comm.getBeaverId(), 2, 2);
			Comm.writeBlock(Comm.getBarrackId(), 2, 1);
			Comm.writeBlock(Comm.getBasherId(), 2, 999);
		} else if (counts[0] == 1) {
			Comm.writeBlock(Comm.getMinerfactId(), 2, 1);
