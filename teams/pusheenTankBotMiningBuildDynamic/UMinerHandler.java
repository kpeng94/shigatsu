		...
		if (rc.isCoreReady()) {
			...
			miningOverride: if (movingToFrontier) {
				if(rc.senseOre(myLoc) >= Mining.ORE_THRESHOLD && rc.canMine()){
					...
					rc.setIndicatorString(1, Clock.getRoundNum() + ": Mining override");
					rc.mine();
				...
				if (numMiners > Mining.ADJ_THRESHOLD) {
					...
					if (ml != null) {
						...
						if (frontier != 0) {
							...
							rc.setIndicatorString(2, Clock.getRoundNum() + ": Moving to frontier at " + MapUtils.decode(frontier & 0xFFFF));
