...
public class Utils {
    ...
    public static void updateEnemyInfo() {
        ...
        Handler.numAttackableEnemies = Handler.attackableEnemies.length;
    ...
    /**
     * Gets the closest unit that is not constructing
     * TODO: think about towers?
     * @param robots
     * @return
     */
    public static RobotInfo closestNonConstructingUnit(RobotInfo[] robots) {
        ...
        for (int i = robots.length; --i >= 0;) {
            ...
            if (distSq < minDist) {
                minDist = distSq;
                closestUnit = robots[i];
            }
        }
        return closestUnit;
    }
    /**
     * Gets the number of allies that can attack enemy at the given location.
     *
     * @param location
     * @return
     * @throws GameActionException
     */
    public static int numAlliesCanAttackEnemyAtLocation(
            MapLocation location) throws GameActionException {
        RobotInfo[] alliedRobots = Handler.rc.senseNearbyRobots(location, 24, Handler.myTeam);
        int count = 0;
    ...
    public static boolean isInTheirHQAttackRange(MapLocation loc) {
        int distSq = Handler.enemyHQ.distanceSquaredTo(loc);
        if (distSq < 25) return true;
        else if (distSq > 25) return false;
        else return (loc.x != Handler.enemyHQ.x) && (loc.y != Handler.enemyHQ.y);
    }
