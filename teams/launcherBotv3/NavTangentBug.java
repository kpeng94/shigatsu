...
public class NavTangentBug {
    ...
    public static void calculate(int bytecodeLimit) {
        ...
        while (Clock.getBytecodeNum() < bytecodeLimit) {
            ...
            if (!tracing) { // not tracing
                ...
                TerrainTile forwardTile = Handler.rc
                        .senseTerrainTile(forwardPos); // Checks forward
                ...
                if (forwardTile == TerrainTile.NORMAL) { // Can move forward
                ...
                } else if (forwardTile == TerrainTile.UNKNOWN) { // Unable to
                                                                 // sense yet
                    closed = true;
                    return;
                    ...
                    if (testDir != null)
                        forwardDir = testDir.opposite();

                    Direction leftDir = forwardDir.rotateLeft(); // Check left
                                                                 // side first
                    for (int i = 7; --i >= 0;) {
                        MapLocation leftPos = cur.add(leftDir);
                        TerrainTile leftTile = Handler.rc
                                .senseTerrainTile(leftPos);
                        if (leftTile == TerrainTile.UNKNOWN) { // Unable to
                                                               // sense yet
                            closed = true;
                            return;
                            ...
                            minDistToDest[encodedLeftPos] = cur
                                    .distanceSquaredTo(dest);
                            distTraveled = 0;
                            tracing = true;
                            break;
                        }
                ...
                MapLocation forwardPos = cur.add(forwardDir);
                TerrainTile forwardTile = Handler.rc
                        .senseTerrainTile(forwardPos);
                if (forwardTile == TerrainTile.UNKNOWN) {
                    closed = true;
                    return;
                } else if (distTraveled >= DIST_MIN
                        && forwardTile == TerrainTile.NORMAL
                        && minDistToDest[encodedCur] > forwardPos
                                .distanceSquaredTo(dest)) { // Discovered
                                                            // shorter distance
                    setNextPosInfo(forwardPos, forwardDir.opposite());
                    nextDir[encodedCur] = forwardDir;
                    tracing = false;
                } else { // try tracing
                    Direction testDir = prevDir[encodedCur].rotateLeft()
                            .rotateLeft();
                    for (int i = 8; --i >= 0;) {
                        MapLocation testPos = cur.add(testDir);
                        TerrainTile testTile = Handler.rc
                                .senseTerrainTile(testPos);
                        if (testTile == TerrainTile.UNKNOWN) { // Unable to
                                                               // sense yet
                            closed = true;
                            return;
    ...
    public static Direction getNextMove() {
        if (Handler.myLoc.distanceSquaredTo(dest) < PROXIMITY)
            return Direction.NONE;
        int checkAmount = 0;
        while (Handler.myLoc.distanceSquaredTo(curFollowing) <= SMART_FUTURE_SENSING
                && checkAmount < CHECK_LIMIT) {
            if (curFollowing.equals(dest))
                return Direction.NONE; // reached destination
