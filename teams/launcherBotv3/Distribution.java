...
public class Distribution {
    ...
    public static final int FIRST_CHAN = 2;
    ...
    public static void initTasks() throws GameActionException {
        ...
        if (taskBlockId == 0) {
            ...
            Comm.writeBlock(taskBlockId, META_CHAN, (FIRST_CHAN << 8)
                    + FIRST_CHAN); // Free block points to 2
        }
    }

    // Runs through tasks round robin
    // Never register more than 254 tasks
    public static void spendBytecodesCalculating(int bytecodelimit)
            throws GameActionException {
        int headTail = Comm.readBlock(taskBlockId, META_CHAN); // 8 bits head, 8
                                                               // heads tail
    ...
    public static boolean executeTask(int bytecodelimit, int task)
            throws GameActionException {
    ...
    public static boolean addTask(int task, int taskId)
            throws GameActionException {
        int headTail = Comm.readBlock(taskBlockId, META_CHAN);
        int head = (headTail >>> 8) & 0x000000ff;
        int tail = headTail & 0x000000ff;
        int next = (tail + 1 > LAST_CHAN) ? FIRST_CHAN : tail + 1;
        if (next == head)
            return false; // No more free channels

        Comm.writeBlock(taskBlockId, tail, task + (taskId << 24));
