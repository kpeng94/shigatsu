...
public class ULauncherHandler extends UnitHandler {
	...
	protected static void init(RobotController rcon) throws GameActionException {
		...
		myWaveNumber = Comm
				.readBlock(Comm.getLauncherId(), Comm.WAVENUM_OFFSET);
		release = false;
		reported = false;
	}
	...
	protected static void execute() throws GameActionException {
		...
		if (enemyTowers.length == 0
				|| (Clock.getRoundNum() >= 1750 && numMyTowers < enemyTowers.length)) {
	...
	private static void checkRelease() throws GameActionException {
		if (rc.getMissileCount() == 0 && release) { // Reset the release
			release = false;
			reported = false;
			int releaseNum = Comm.readBlock(Comm.getLauncherId(),
					Comm.LAUNCHER_RELEASE_OFFSET);
			Comm.writeBlock(Comm.getLauncherId(), Comm.LAUNCHER_RELEASE_OFFSET,
					releaseNum - 1);
		} else if (!release && rc.getMissileCount() == 5) {
			if (!reported) { // Report that launcher is ready
				int releaseNum = Comm.readBlock(Comm.getLauncherId(),
						Comm.LAUNCHER_RELEASE_OFFSET);
				Comm.writeBlock(Comm.getLauncherId(),
						Comm.LAUNCHER_RELEASE_OFFSET, releaseNum + 1);
				reported = true;
			}
			if (Count.getCount(Comm.getLauncherId()) == Comm.readBlock(
					Comm.getLauncherId(), Comm.LAUNCHER_RELEASE_OFFSET)) { // Set the release if everyone is ready
				release = true;
			}
		}
	}
	...
	private static void rallyCode() throws GameActionException {
		...
		NavTangentBug.calculate(2500);
		// TODO (kpeng94): maybe change this so that when you're in a really
		// close region, bug
		// around instead
		if (rc.isCoreReady()) {
		...
		numberOfLaunchersRallied = Count.getCountAtRallyPoint(
				Comm.getLauncherId(), myWaveNumber);
		// rc.setIndicatorString(1, "My wave number is: " + myWaveNumber +
		// ". The number of tanks that rallied is given by: " +
	...
	public static boolean decideAttack() {
		...
		return (enemies.length > 0 || closestLocation.distanceSquaredTo(myLoc) <= 35)
				&& (release || (reported && rc.getMissileCount() == 5));
	}
	...
	public static void attackAndMove() throws GameActionException {
		...
		if (closestEnemy != null && rc.isCoreReady()) {
			if (closestEnemy.location.distanceSquaredTo(myLoc) <= 15) {
				NavSimple.walkTowardsSafe(closestEnemy.location
						.directionTo(myLoc));
			}
		}
	}
