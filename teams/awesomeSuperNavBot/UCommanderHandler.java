...
import awesomeSuperNavBot.NavSafeBug.traceDir;
		...
		if (!flashSuccess) { // did not flash
			if (missileClose > 0 || missileSeen >= 3 || immediateDamage >= rc.getHealth() / 20) { // TODO: change constant?
				...
				if (towardsDir != Direction.NONE) {
					rc.setIndicatorString(2, Clock.getRoundNum() + " walking forward " + target.location + " " + towardsDir);
					rc.move(towardsDir);
				}
			}
		}
		...
		if ((Clock.getRoundNum() - lastSafeNavTurn > SAFE_NAV_THRESHOLD) && (closestTower == null || myLoc.distanceSquaredTo(closestTower) > TOWER_THRESHOLD) && myLoc.distanceSquaredTo(enemyHQ) > HQ_threshold) { // Out of range, resume tangent bugging
			...
			if (prevNavTangent) {
				...
				if (NavTangentBug.tracing) {
					NavSafeBug.tracing = traceDir.LEFT;
